...
object ClassificationPipeline {
  def main(args: Array[String]) {
    ...
    /** Step 1
      ...
      * ... ...
      */
    val parsedRDD = sc.textFile("data/RandomForestData/data_banknote_authentication.txt").map(_.split(",")).map(eachRow => {
    ...
    /**
      ...
      */
//    predictionResultDF.select("f0","f1","f2","f3","label","predictedLabel").show(100)
